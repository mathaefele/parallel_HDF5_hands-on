...
PROGRAM PHDF5_EX
     ...
     INTEGER(HID_T) :: file, dataset, dataspace, filespace, memspace, plist_id1, plist_id2
     ...
     integer :: size_x_array, size_y_array
     ...
     var_nx = NX
     var_ny = NY
     call init(var_nx, var_ny, mpi_id, size, size_x, size_y, offset_x, offset_y, size_x_array, size_y_array, dat) 
     ...
     !HDF5 file creation
     CALL h5pcreate_f(H5P_FILE_ACCESS_F, plist_id1, status)
     CALL h5pset_fapl_mpio_f(plist_id1, MPI_COMM_WORLD, MPI_INFO_NULL, status)
     CALL h5fcreate_f(filename, H5F_ACC_TRUNC_F, file, status, access_prp = plist_id1)
     ...
     !Actual data IO
     CALL h5pcreate_f (H5P_DATASET_XFER_F, plist_id2, status)
     CALL h5pset_dxpl_mpio_f (plist_id2, H5FD_MPIO_COLLECTIVE_F, status)
     el_count(1) = size_x
     el_count(2) = size_y
     start(1) = offset_x
     start(2) = offset_y
     ...
     el_count(1) = size_x_array
     el_count(2) = size_y_array
     CALL h5screate_simple_f(rank, el_count, memspace, status)
     el_count(1) = size_x
     el_count(2) = size_y
     start(1) = 1 
     start(2) = 1 
     CALL H5sselect_hyperslab_f(memspace, H5S_SELECT_SET_F, start, el_count, status)

     CALL h5dwrite_f(dataset, H5T_NATIVE_INTEGER, dat, el_count, status, &
     mem_space_id=memspace, file_space_id=filespace, xfer_prp = plist_id2)
     ...
     contains
subroutine init(NX, NY, mpi_id, size, size_x, size_y, offset_x, offset_y,&
  size_x_array, size_y_array, dat)
  integer, intent(in) :: NX, NY, mpi_id, size
  integer, intent(out) :: size_x, size_y, offset_x, offset_y, size_x_array, size_y_array 
  integer :: i,j
  integer, dimension(:,:), allocatable :: dat
  ...
  size_x_array =  size_x + 2
  size_y_array =  size_y + 2
  offset_x = mod(mpi_id,nb_proc_per_dim) * (size_x)
  offset_y = int(mpi_id/nb_proc_per_dim) * (size_y)

  allocate(dat(size_x_array,size_y_array))

  do j=-1, size_y_array-2
    do i=-1, size_x_array-2
      if(i+offset_x<0 .or. j+offset_y < 0 .or. i+offset_x >= NX .or. j+offset_y >= NY) then
        dat(i+2,j+2) =0
      else
        dat(i+2,j+2) = i+offset_x + (j+offset_y)*NX
      end if
    end do
  end do
